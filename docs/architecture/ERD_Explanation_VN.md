...
3. **Optimistic Concurrency Control**: Hầu hết các bảng chính đều có cột `binary RowVersion`. Cột này được ORM dùng để xử lý xung đột đồng thời (khi 2 user cùng update 1 dòng dữ liệu tại 1 thời điểm).
...
- **`testgen_TestCases`**: Thông tin kịch bản kiểm thử độc lập (ví dụ: "Test đăng nhập sai password"). Một TestSuite có nhiều TestCase. Điểm đặc biệt:
  ...
  - Cột `Priority`, `IsEnabled`, `OrderIndex`: Dùng để quản lý luồng thực thi ưu tiên và có loại bỏ ca test này trong lần chạy (Run) sắp tới hay không.
...
- **`sub_PaymentIntents`** & **`sub_PaymentTransactions`**: Theo dõi quy trình thanh toán Checkout.
  - `PaymentIntents`: Phác thảo ý định thanh toán. Khởi tạo một order, sinh `CheckoutUrl` cho user nhấn vào. Quá giờ (`ExpiresAt`) thì hủy.
...
- **`sub_UsageTrackings`**: Tracking mức độ dùng tài nguyên của User (Đã tạo bao nhiêu Project, Dùng bao nhiêu token AI) trong chu kỳ tháng đó (`PeriodStart` -> `PeriodEnd`) để so sánh với bảng `sub_PlanLimits`. Nếu vượt quá giới hạn (limit), hệ thống sẽ chặn.
...
- **`llm_LlmInteractions`**: Ghi lại mọi tương tác (Prompt/Response) dưới dạng Log.
...
- **`llm_LlmSuggestionCaches`**: AI Caching layer để giảm tải API Cost.
  - Khi 1 User yêu cầu AI sinh test cho API `/api/v1/users`, nếu `CacheKey` này (có thể là hash của cấu trúc input) đã có trong DB và chưa `ExpiresAt`, hệ thống lấy luôn `Suggestions` JSON trả về mà không gọi thêm LLM. -> Đây là một Pattern tối ưu rất tốt để trả lời các câu hỏi khó của giảng viên về chi phí duy trì dự án sinh viên.
...
2. **Cách phối hợp Event-Driven (Cực kỳ "ăn điểm" kỹ thuật)**: 
   - Trình bày về **Outbox Pattern**. Có thể đặt vấn đề với giảng viên: "Tưởng tượng khi có user thanh toán xong Subscription, làm sao để cộng số lượt sử dụng LLM mà hệ thống không bị crash lúc kết nối mạng lỗi?". Việc bạn thiết kế bảng `OutboxMessages` giải quyết nhược điểm lưu trữ event trên hệ thống phân tán rất tinh tế.
