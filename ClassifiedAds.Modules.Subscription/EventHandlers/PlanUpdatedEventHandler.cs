...
public class PlanUpdatedEventHandler : IDomainEventHandler<EntityUpdatedEvent<SubscriptionPlan>>
{
    ...
    public PlanUpdatedEventHandler(ICurrentUser currentUser,
    ...
    public async Task HandleAsync(EntityUpdatedEvent<SubscriptionPlan> domainEvent, CancellationToken cancellationToken = default)
    {
        ...
        await _outboxMessageRepository.AddOrUpdateAsync(new OutboxMessage
        {
            EventType = EventTypeConstants.PlanUpdated,
            TriggeredById = _currentUser.IsAuthenticated ? _currentUser.UserId : Guid.Empty,
            CreatedDateTime = domainEvent.EventDateTime,
            ObjectId = domainEvent.Entity.Id.ToString(),
