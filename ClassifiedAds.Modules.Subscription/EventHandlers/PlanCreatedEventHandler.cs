...
public class PlanCreatedEventHandler : IDomainEventHandler<EntityCreatedEvent<SubscriptionPlan>>
{
    ...
    public PlanCreatedEventHandler(ICurrentUser currentUser,
    ...
    public async Task HandleAsync(EntityCreatedEvent<SubscriptionPlan> domainEvent, CancellationToken cancellationToken = default)
    {
        ...
        await _outboxMessageRepository.AddOrUpdateAsync(new OutboxMessage
        {
            EventType = EventTypeConstants.PlanCreated,
            TriggeredById = _currentUser.IsAuthenticated ? _currentUser.UserId : Guid.Empty,
            CreatedDateTime = domainEvent.EventDateTime,
            ObjectId = domainEvent.Entity.Id.ToString(),
